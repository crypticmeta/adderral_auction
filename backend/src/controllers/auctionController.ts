  ...
  try {
    ...
    if (auction) {
      ...
      io.to(`auction:${auctionId}`).emit('auction:update', {
        ...
        tokensOnSale: auction.tokensOnSale,
...
export const createAuction = async (req: Request, res: Response) => {
  try {
    const { totalTokens, tokensOnSale, ceilingMarketCap, startTime, endTime, minPledgeSats, maxPledgeSats } = req.body;
    
    if (!totalTokens || !tokensOnSale || !ceilingMarketCap || !startTime || !endTime) {
      return res.status(400).json({ message: 'Missing required auction fields (totalTokens, tokensOnSale, ceilingMarketCap, startTime, endTime)' });
    }
    
    const auction = await prisma.auction.create({
      data: {
        totalTokens: parseFloat(totalTokens),
        tokensOnSale: parseFloat(tokensOnSale),
...
export const resetAuction = async (req: Request, res: Response) => {
  try {
    ...
    // 3) Reseed minimal data (aligns with prisma/seed.ts for totals; no mock users/pledges)
    //    Admin user
    ...
    //    No mocked users or pledges are created here

    //    New 72h auction
    const now = new Date();
    const endTime = addHours(now, 72);

    //    Mirror prisma/seed.ts totals by SEED_MODE
